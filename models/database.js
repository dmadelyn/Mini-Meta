...
var getArticleInfo = function(articleID, callback) {
  var params = {
    KeyConditions: {
      id: {
        ComparisonOperator: 'EQ',
  ...
  db.query(params, function(err, data) {
    if (err || !data.Items[0]) {
      ...
      var outputJson = {
        articleID : articleID,
        authors : result.authors.S,
        category : result.category.S,
        date : result.date.S,
        headline : result.headline.S,
        short_description : result.short_description.S,
        url : result.url.S
      }
...
var getRecommendedArticle = function(username, callback) {
  var dateStr = format('yyyy-MM-dd', new Date());
  var params = {
  ...
  db.query(params, function(err, data) {
    if (err || !data.Items) {
      ...
      for (let i = 0; i < data.Items.length; i++) {
        sum += parseFloat(data.Items[i].score.N);
      }
      var output = [];
      data.Items.forEach((item) => {
        var json = {
          articleId: item.articleId.N,
          score: (parseFloat(item.score.N) / sum),
        }
        output.push(json);
      })
      callback(err, output);
...
var deleteRecommendedArticle = function(username, articleId, callback) {
  var params = {
    TableName: "adsorptionScore",
    Key: {
      'username' : {S: username},
...
var postArticleLike = function(username, articleId, callback) {
  var params = {
    Item : {
      'username' : {S: username},
      'articleId' : {N: articleId}
...
var getArticleLike = function(username, articleId, callback) {
  var params = {
    TableName : 'articleLikes',
    KeyConditions: {
      username: {
...
var deleteArticleLike = function(username, articleId, callback) {
  var params = {
    TableName: "articleLikes",
    Key: {
      'username' : {S: username},
...
var database = { 
  ...
  getArticleScore: getArticleScore,
  getArticleInfo: getArticleInfo,
  getRecommendedArticle: getRecommendedArticle,
  deleteRecommendedArticle: deleteRecommendedArticle,
  postArticleLike: postArticleLike,
  getArticleLike: getArticleLike,
  deleteArticleLike: deleteArticleLike
};
